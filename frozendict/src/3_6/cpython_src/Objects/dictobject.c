                ...
                PyObject ***value_addr, Py_ssize_t *hashpos)
{
    ...
    for (size_t perturb = hash; ix != DKIX_EMPTY;) {
        perturb >>= PERTURB_SHIFT;
        i = (i << 2) + i + perturb + 1;
        ix = dictkeys_get_index(mp->ma_keys, i & mask);
    }
...
static PyObject *
dict_get(PyDictObject *self, PyObject *args)
{
    PyObject *key;
    ...
    ix = (self->ma_keys->dk_lookup) (self, key, hash, &value_addr, NULL);
    if (ix == DKIX_ERROR)
        return NULL;
    if (ix == DKIX_EMPTY || *value_addr == NULL) {
        val = default_value;
...
dictiter_reduce(dictiterobject *di, PyObject *Py_UNUSED(ignored))
{
    ...
    for(;;) {
        PyObject *element = 0;
        if (Py_TYPE(di) == &PyFrozenDictIterItem_Type)
